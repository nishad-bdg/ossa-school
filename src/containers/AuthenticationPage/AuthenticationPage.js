...
export class AuthenticationPageComponent extends Component {
  ...
  render() {
    ...
    const uploadedStaffPhoto = this.props.image;

    const profileImage = image || { imageId: null };
    ...
    //   console.log('on image upload', file);
    // };

    const handleSubmitSchoolSignup = values => {
    ...
    try {
      ...
      userType = splitCurrentUrl[1].split('=')[1];
    } catch (_) {}
    ...
    const initialValues = {
      ...
      schoolActivities: [],
      topStudentNationalities: [],
      classroomEquipments: [],
    };

    const userSignupForm =
      userType && userType.toLowerCase() === 'school' ? (
        <SignupFormSchool
          initialValues={initialValues}
          className={css.signupForm}
          onSubmit={handleSubmitSchoolSignup}
          inProgress={authInProgress}
          onOpenTermsOfService={() => this.setState({ tosModalOpen: true })}
...
const { bool, func, object, string, oneOf, shape } = PropTypes;
...
AuthenticationPageComponent.propTypes = {
  ...
  uploadInProgress: propTypes.error,
  uploadImageError: bool,
  submitLogin: func.isRequired,
  submitSignup: func.isRequired,
  submitSchoolSignup: func.isRequired,
...
const mapDispatchToProps = dispatch => ({
  ...
  submitSchoolSignup: params => dispatch(schoolSignup(params)),
