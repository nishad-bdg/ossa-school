...
// school sign up
export const schoolSignup = params => (dispatch, getState, sdk) => {
  if (authenticationInProgress(getState())) {
    return Promise.reject(new Error('Login or logout already in progress'));
  }
  dispatch(signupRequest());
  ...
  const createUserParams = isEmpty(rest)
    ? { email, password, firstName: 'School First Name', lastName: 'School Last Name' }
    : {
        email,
        password,
        firstName: 'School First Name',
        lastName: 'School Last Name',
        publicData: { ...rest },
      };

  console.log('created params', createUserParams)
