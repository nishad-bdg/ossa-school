...
For more information of the configuration, see the Install flex cli in your system
https://www.sharetribe.com/docs/introduction/getting-started-with-flex-cli/

[Flex CLI Login](https://www.sharetribe.com/docs/introduction/getting-started-with-flex-cli/) use
API key. After login click right top corner API Keys

```sh
flex-cli search -m   warple-test                       # search schema list
flex-cli search set --schema-for userProfile --key city --type enum --scope public -m warple-test      # add city in search list for userProfile Schema
flex-cli search set --schema-for userProfile --key schoolLocation --type enum --scope public -m warple-test   # add city in  userProfile Schema search list
flex-cli search set --key category --type enum --scope public -m warple-test
# add category in  list Schema search list
```
